# 海盗派黑帆测试官：从理解需求到取得测试证据

拿到一份 PRD，你可能很快就能列出正常、异常和边界场景。黑帆测试官关心更难的部分：有没有漏掉关键业务？哪些问题值得先查？用例即使全部通过，能否说明用户的事情真的办成了？

这篇教程用一个票据助手的例子，带你和黑帆测试官一起完成需求分析、测试设计、执行调查与风险更新。你会学到怎样使用 SFDIPOT、MFQ、PPDCS，也会知道什么时候不用它们。

阅读时不必记住全部 Skill 名称。通常从 `test-privateer` 开始，说明你的目标和已有材料即可。已有明确测试点、用例或代码时，也可以直接进入对应工作。AI 已形成研发、修复或实施方案时，`checking-solutions` 会在交付或实施前检查样例耦合、作用点和证据链；SDD 变更包会额外检查规格、任务和归档契约。
...
| AI 已经给出研发或修复方案 | “质检这份方案是否只针对当前 badcase，作用点是否对症，验证能否证明目标行为。” |
...
这套工具包含 23 个 Skill，主入口会按需要调用其他组件。安装全集：
...
开始完整需求分析时，先选独立分析或项目任务。独立分析在对话或指定位置交付，也可参考相关代码；项目任务先收集项目资料、建立或补齐测试地图，再维护需求测试资产。你已说明工作方式时不用重复确认，尚未说明时黑帆测试官会先问清。项目关联不明确时一起核实，避免把无关仓库纳入分析。

`npx skills update` 用于更新已安装的 Skill；Skill 内的 references 模板属于配套资料。新增独立 Skill 后，按仓库说明重新执行全集安装命令。

## 2. 理解这套方法的出发点

黑帆测试官重视在有限时间里持续学习产品。一次测试可能验证了预期，也可能暴露一个此前没想到的问题；两者都会影响下一步安排。它不负责把需求抄成清单，而是带着当前任务登船，寻找最值得取得的下一条证据。
...
如果一个未知会改变测试范围、状态模型、判定依据或风险重点，黑帆测试官会先提出最需要你决定的问题，给出基于证据的推荐、取舍和影响。等待回答时，它可以继续查找不依赖该决定的事实，但不会先写一套完整模型再把问题埋在文档末尾。

例如 PRD 写“历史任务不受配置修改影响”，但没有说明排队任务是否算历史任务，合适的交流是：

```text
🙋 请你确认｜历史任务何时冻结配置？
推荐：任务创建时固定配置版本；排队、执行和失败重试沿用该版本。
取舍：普通解绑只影响新任务；若需要立即吊销旧凭证，要另定义紧急撤销规则。

🔎 判断依据｜当前理解
排队任务、执行中任务和失败重试都可能跨越配置修改；“创建或执行时快照”会得到不同测试预期。

➡️ 确认后做什么｜下一步
根据选择更新时间模型、风险和测试条件，再继续分析其他不依赖本决定的内容。
```

只有你明确要求“先出草案”时，才会在未决规则上展开完整模型；草案中的推荐仍不是已确认规则。信息已经充分时，黑帆测试官直接分析，不为了形式提问。
...
| 编号 | 类型 | 测试点 | 观察重点 |
| --- | --- | --- | --- |
| T-001 | M | 为当前员工生成候选 | 候选与票据数据相关，且属于允许关联的范围 |
| T-002 | M | 用户确认一个候选 | 操作前后选中目标一致，未确认不写入 |
| T-003 | M | 写入费用关联 | 费用记录与票据形成正确关联 |
| T-004 | F | 另一员工存在同金额、同日期费用 | 不能因表面数据相同而跨员工关联 |
| T-005 | F | 候选返回后，费用状态发生变化 | 确认时重新判断是否允许关联 |
| T-006 | Q | 候选存在歧义 | 用户能看懂区别并选择，不能只靠匹配分数猜测 |
| T-007 | Q | 写入响应超时 | 用户能知道结果是否确定，后续操作不造成重复费用 |
...
模型细化已有测试项时沿用编号。本例将 T-007 拆成“查询到原结果”和“确认未写入后重试”两项，新分配 T-008、T-009；旧 T-007 保留“已拆分至 T-008、T-009”的去向，退出当前待设计清单：

| 测试条件 | 来源 | 要验证的情况 |
| --- | --- | --- |
| T-002 | 有效确认路径及其前置状态 | 未确认不写入；确认后关联目标与用户选中的有效候选一致 |
| T-005 | 失效分支 | 候选关闭后，旧页面确认不能继续写入 |
| T-008 | 超时后确认成功 | 响应丢失不会导致重复关联或新建费用 |
| T-009 | 超时后确认未写入 | 重新提交前仍检查目标有效性 |
...
| --- | --- | --- | --- | --- |
| C-001／T-002、T-003 | A 的票据、A 的有效费用 E1 | 选择 E1，先检查未确认时的关联；再确认并查询结果 | 未确认不写入；确认后页面目标与实际关联均为 E1，结果可查询 | 版本、账号与观察入口待核实 |
| C-002／T-005 | A 已看到自己的有效费用 E1；可控制其状态 | 关闭 E1 后，从旧页面确认 | 拒绝写入 E1，提示重新选择，费用数据不变 | 版本与状态控制方式待核实 |
| C-003／T-008 | 独立票据与有效费用；可模拟响应丢失 | 写入成功后丢弃响应，再按恢复流程操作 | 找回原结果，不新增重复关联或费用 | 故障模拟与结果查询待准备 |
| C-004／T-004 | A、B 各有同金额、同日期费用；A 登录 | 上传 A 的票据，检查候选并尝试关联 | 不能关联 B 的费用；接口与实际写入均遵守归属规则 | 数据与权限观察待核实 |

本轮示例把 T-002、T-003 关联到 C-001，T-005 关联到 C-002，T-008 关联到 C-003，T-004 关联到 C-004；编号不要求一一对应。T-001、T-006、T-009仍保留待设计安排，T-007只保留拆分去向。Agent应把这些关联同步回主分析。
...
看到异常时，先记录观察、判定依据和影响；用户要求定位原因时，再沿着“产生原因 → 放大条件 → 防护失效”追查。每个原因假设要有可观察预测、区分动作和结果，不能只凭时间先后、复现成功或修复后通过就宣布根因。

例如 Agent 反复点击同一个按钮：重复保护持续拒绝，可能解释循环为什么没有停止；它还没有解释模型为什么继续生成同一动作。此时应继续对照动作前后状态、事务记录和下一轮模型输入，或者明确留下未解释范围。

出现复杂多条件事故时，可以按需使用发生／未发生对照、5 Why、鱼骨图、故障树、差分调试或控制反馈分析。方法用于生成和检验解释，不要求每次都画图或给出唯一根因。

## 7. 执行之后，怎样形成可信结论

具备执行条件并已获授权后，使用 `running-test-sessions`。记录操作和实际观察，随后判断它们说明了什么。

下面是假设执行 C-002 后得到的教学记录：
...
需要评审元素、样式、交互、无障碍或可用性时，可直接使用 `reviewing-ui-ux`，也可让 `test-privateer` 按当前判断缺口进入专项：

```text
使用 reviewing-ui-ux，评审费用关联失败后的界面体验。
用户是首次使用的报销员工，已有设计稿和一次失败操作记录。
产品要求失败后保留已选费用，并说明怎样继续。
请沿用户任务选择方法，区分有依据的问题、风险和未观察的行为。
本轮只评审这些材料，不执行产品测试。
```

本例应先用状态与恢复分析核对失败反馈、选择保留和后续动作，再以认知走查判断员工是否理解如何继续。如果记录只有错误画面，可以判断提示是否清楚；输入是否保留，需要失败前后证据。若争议在员工实际能否理解，则补无提示的用户任务观察。无需因此扩成六类质量清单或重新建立项目地图。

新设计明确批准的颜色变化先按预期变化解释；窄屏长文案遮住关键操作则继续检查用户影响。视觉差异、规则扫描与操作成功各自支持不同主张。专项提供方法及观察要求；具体用例、获准执行和自动化设计仍由原有技能承接。
...
编号规则的权威位置是 [artifact-routing](../skills/test-privateer/references/artifact-routing.md#统一编号与追溯)。项目覆盖节点用 N-001，风险用 R-001，测试项用 T-001，具体用例用 C-001。业务分类与 M／F／Q 类型写在标题或字段里，不各自重新编号。

同一需求的分析、依据和用例文件共享编号范围；跨需求或引用项目风险时带所属文件与完整编号、短名。同一测试项从候选细化为TCON不因换Skill改号，真正拆分/合并才记录新旧号关系。排序和改名不改号，撤回编号不复用。原始执行证据保留原号；已有外部权威编号继续沿用。

成稿Agent统一分配新增编号，复核定义、引用和关联状态。用户无需逐条核对编号，也不需要为自由Markdown安装编号解析器。
...
完整分析、模型或用例集交付前，成稿Agent先使用 `challenging-test-designs` 自校并修正，再检查关联内容。复杂或跨文件工作在能力允许时借助独立审阅，意见由Agent查证处理；已有依据能解决的文档错误不退回用户确认。真正的业务规则、范围或授权选择才需要人决定。

用户想进一步判断分析价值时，可以从下面几个问题入手：
...
也可以直接调用 `challenging-test-designs` 审查既有分析或用例。仅要求审查时只返回有依据的意见；要求生成或修改时，Agent负责修正本轮产物并复读影响范围。复核应保留已有正确观察与执行限制，不为“审得严格”不断增加范围。

## 11. 需要时再查这张 Skill 索引

| 当前工作 | Skill | 主要结果 |
| --- | --- | --- |
| 找到合适起点 | `test-privateer` | 当前任务和下一项活动 |
...
| 评审界面与使用体验 | `reviewing-ui-ux` | 方法选择、有依据的体验发现、证据边界与下一探查 |
