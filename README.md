...
关键业务规则、配置时点、判定依据或范围不清时，黑帆测试官会先提出具体问题，给出推荐答案、依据和测试影响；未决答案不会被写成既定规则。你可以直接这样说：

```text
使用 test-privateer 分析这份 PRD。
如果有会改变测试范围或判定依据的未知，请先给我推荐方案和影响，再继续不依赖它的调查。
本轮只做分析，不设计具体用例，不执行产品测试。
```

遇到异常或需要查根因时，说明现象、环境、时间和已有证据：

```text
使用 test-privateer 调查这个重复动作问题。
请区分异常产生、循环放大和防护失效；不要把止损措施直接当成根因。
```

根因调查会保留竞争解释和未解释范围。问题已经发生时，补充日志、原始输入、输出、状态变化和可复现条件，比只说“还是失败了”更容易得到可验证结论。
